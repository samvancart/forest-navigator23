...
def get_data_list(lon, lat):
    request = build_api_call(lon=lon , lat=lat,properties=['silt'], values=['mean'])
...
def get_data_content(lon, lat):
    request = build_api_call(lon=lon , lat=lat,properties=['silt', 'clay', 'sand'], values=['mean'])
    res = get_data(request)
    return res

content = get_data_content(def_lon,def_lat)
...
# df.to_csv('data/csv/silt_soildata.csv',index = False)
# print(df)
